...
g1 <- ggplot(data, aes(x = nodes, y = time.max, fill = type)) +
  geom_bar(stat='identity',  position=position_dodge(width=0.7), width=0.6) +
  labs(linetype = "operation") +
  labs(colour = "config") +
  labs(y = "time [s]") +
  labs(x = "#nodes") +
  scale_fill_discrete(labels=c("cosma gpu (COSTA)",  "cosma cpu (COSTA)",  "libsci",  "libsci gpu",  "mkl")) +
...
ggsave("cp2k-bar-timings.pdf", g1, width=5, height=3.5,  units="in")
